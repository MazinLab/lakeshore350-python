...
class TemperatureRecorder:
    def __init__(self):
        ...
        self.column_widths = [28, 12, 12, 17, 17, 17, 20, 18, 16, 18, 16, 17, 16, 17, 16, 18, 19]
    ...
    def _get_next_csv_filename(self):
        ...
        base_filename = f"temps/{self.start_date}_temperature_log_adr_test.csv"
        ...
        counter = 1
        while True:
            numbered_filename = f"temps/{self.start_date}_temperature_log_adr_test_{counter}.csv"
    ...
    def print_formatted_row(self, data_row):
        ...
        for i, value in enumerate(data_row):
            ...
            if i == 6 and isinstance(value, float):
                ...
                formatted_value = f"{value:.3f}"
            elif i == 15 and isinstance(value, float):
                # Input B resistance (4 decimal places)
                formatted_value = f"{value:.4f}"
            elif i == 16 and isinstance(value, float):
                # Input D1 resistance (4 decimal places)
                formatted_value = f"{value:.4f}"
    ...
    def get_temperatures(self):
        ...
        try:
            ...
            # Switch voltage (Input D2)
            switch_voltage = self.temp_reader.read_sensor('D2')
            if isinstance(switch_voltage, float):
                switch_temp = convert_pump_voltage_to_temperature(switch_voltage)
            else:
                switch_temp = None
            ...
            if isinstance(d5_voltage, float):
                ...
                d5_temp = None
            # Input B resistance (raw sensor output)
            input_b_resistance = self.temp_reader.read_sensor('B')
            # Input D1 resistance (raw sensor output)
            input_d1_resistance = self.temp_reader.read_sensor('D1')
            ...
            return {
                ...
                "d5_temp": d5_temp,
                "input_b_resistance": input_b_resistance,
                "input_d1_resistance": input_d1_resistance
            }
    ...
    def format_csv_row(self, data):
        ...
        try:
            row = [
                ...
                data["d5_voltage"],
                data["d5_temp"],
                data["input_b_resistance"],
                data["input_d1_resistance"]
            ]
            return row
        except Exception as e:
